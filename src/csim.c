...
{
  printf(
    "Examines given input file (CellML or RDF/XML) and executes the\n"
...
int main(int argc,char* argv[])
{
  ...
  if (optind < argc) inputURI = getAbsoluteURI(argv[optind]);
  ...
  if (simulation)
  {
	  if (simulationIsValidDescription(simulation))
	  {
		  ...
		  {
			  DEBUG(0,"main","Ran the simulation (%s) successfully\n",
					  simulationName);
		  }
		  else
		  {
			  ERROR("main","Unable to run the simulation: %s\n",
					  simulationName);
			  code = ERR;
		  }
		  if (simulationName) free(simulationName);
	  }
	  else
	  {
  ...
  else DEBUG(0,"main","No simulations (or not exactly one simulation found), probably something missing\n");
  ...
  struct CellMLCodeManager* codeManager)
{
  ...
  if (simulation && simulationIsValidDescription(simulation))
  {
    ...
    struct IntegratorUserData* userData =
      CreateIntegratorUserDataForModel(codeManager,modelURL);
    if (userData)
    {
      DEBUG(0,"runSimulation","Got the user data for the model: %s\n",
        modelURI);
      integratorUserDataInitialise(userData,
        simulationGetBvarStart(simulation));
      struct Integrator* integrator = CreateIntegrator(simulation,userData);
      if (integrator)
      {
          ...
          startTimer(timer);
          double integrationTimes[3] = {0.0,0.0,0.0};
          double dataStoreTimes[3] = {0.0,0.0,0.0};
          while (1)
          {
            DEBUG(5,"runSimulation","tout = "REAL_FORMAT"\n",tout);
            double t;
            iout++;
            TIME_FUNCTION_CALL(integrationTimes,intTimer,code,integrate,
              integrator,userData,tout,&t);
            if (code == OK)
            {
              ...
              {
                DEBUG(0,"runSimulation","Error appending integration results "
                  "at time: "REAL_FORMAT"\n",tout);
                break;
              }
            }
          ...
          MESSAGE("    (integration "REAL_FORMAT" s)\n",integrationTimes[2]);
          MESSAGE("    (data store  "REAL_FORMAT" s)\n",dataStoreTimes[2]);
          MESSAGE("  CPU time        : "REAL_FORMAT" s "
      ...
      else ERROR("runSimulation","Error creating integrator\n");
      DestroyIntegratorUserData(&userData);
    }
    else ERROR("runSimulation","Error getting the user data for the "
      "model: %s\n",modelURL);
    if (modelURI) free(modelURI);
    if (modelURL) free(modelURL);
  }
