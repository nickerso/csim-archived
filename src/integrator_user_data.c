  ...
  struct CellMLCodeManager* codeManager,const char* uri)
{
  ...
  if (codeManager && uri)
  {
    ud = (struct IntegratorUserData*)malloc(sizeof(struct IntegratorUserData));
    if (ud)
    {
      /* initialise */
      ...
      if ((ud->methods = cellmlCodeManagerGetMethodsForModel(codeManager,uri)))
      {
        /* allocate memory */
  ...
  else DEBUG(0,"CreateIntegratorUserDataForModel","Invalid argument(s)\n");
  return(ud);  
}
...
int DestroyIntegratorUserData(struct IntegratorUserData** ud_ptr)
{
  ...
  if (ud)
  {
    if (ud->methods) DestroyCellMLMethods(&(ud->methods));
    if (ud->BOUND) free(ud->BOUND);
    if (ud->CONSTANTS) free(ud->CONSTANTS);
    if (ud->STATES) free(ud->STATES);
    if (ud->ALGEBRAIC) free(ud->ALGEBRAIC);
    if (ud->RATES) free(ud->RATES);
    free(ud);
